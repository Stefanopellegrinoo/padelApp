-- ── promote_guest deja de escribir entries.seed_position (C37) ─────────────
-- Restatement QUIRÚRGICO de la definición viva (0048_promote_guest_award_lines
-- .sql), verificada contra `pg_proc.prosrc` antes de copiarla — no contra el
-- archivo, que es como se copia de la migración equivocada.
--
-- Cambian SÓLO tres cosas, todas la misma: se cae `v_at` del declare, se cae
-- el `select max(seed_position) + 1` que lo llenaba, y el `update` que hacía
-- el flip GUEST -> SQUAD pone `seed_position = null`. Todo lo demás —los
-- guards del reparto, el casillero de puntos que crece la config, el
-- `discipline_entries` con su `p_before`, la copia de awards y de
-- `award_lines` (REQ-D10-3)— es byte a byte lo que ya estaba.
--
