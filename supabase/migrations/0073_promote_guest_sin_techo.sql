-- ── promote_guest se queda sin el techo de 12 (docs/plan-piso-y-techo-del-
--    plantel.md, Task 3) ───────────────────────────────────────────────────
-- Restatement QUIRÚRGICO de la definición viva (`0062_promote_guest_no_entries
-- _seed.sql`), verificada contra `pg_proc.prosrc` antes de copiarla — no
-- contra el archivo, que es como se copia de la migración equivocada.
--
-- Cambia UNA sola cosa: adentro de `if v_pair_size = 1`, se cae el guard
-- `if v_squad + 1 > 12 then raise ...`. Todo lo demás —la lectura de
-- `squadSize`/`points` como jsonb con su chequeo de tipo, el resto de los
-- guards, el `discipline_entries` con su `p_before`, la copia de awards y de
-- `award_lines`— es byte a byte lo que ya estaba.
--
-- Por qué se va, las tres razones medidas (docs/plan-piso-y-techo-del-
-- plantel.md, "Lo medido"):
--   1. Dejó de proteger nada el 24/08, cuando el techo de PARTIDOS de la
--      fecha pasó a ser trabajo de `config.maxMatches`/`defaultMaxMatches`
--      (`core/constants.ts`) — este guard medía CABEZAS de plantel, no
--      partidos, y ese trabajo ya lo tiene otro.
--   2. Estaba DUPLICADO entre TypeScript (el techo de plantel de
--      `core/constants.ts`) y acá — la misma regla de producto escrita dos
--      veces, con tripwire cruzado (`core/constants.test.ts`) para que no
--      divergieran. El TypeScript ya se borró; esta migración borra la
--      otra mitad.
--   3. Es código MUERTO en producción hoy: `main` está antes de `0015` y no
--      existe ahí ni una sola disciplina con `pair_size = 1` — la rama que
--      declara este guard nunca corrió con datos reales.
--
