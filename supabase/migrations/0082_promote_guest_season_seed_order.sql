-- ── promote_guest también escribe season_seed_order (torneo-multi-
--    disciplina, tanda 1) ─────────────────────────────────────────────────
-- Restatement quirúrgico de la definición viva (0073_promote_guest_sin_
-- techo.sql), verificada contra `pg_proc.prosrc` antes de copiarla — no
-- contra el archivo, que es como se copia de la migración equivocada.
--
-- Cambia UNA sola cosa: se suma `v_season_at` al declare, y un insert más en
-- `season_seed_order` (0080_season_seed_order.sql) junto al de
-- `discipline_entries` de siempre. Todo lo demás —los guards del reparto,
